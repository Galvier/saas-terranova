...
-- Corrigir a função broadcast_notification_from_template para adicionar cláusula ELSE
...
DECLARE
  ...
  metric_info RECORD;
BEGIN
  -- Buscar o template (permitir templates inativos para casos temporários)
  SELECT * INTO template_record
  FROM notification_templates
  WHERE id = template_id_param;
  
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Template not found with id: %', template_id_param;
  END IF;
  ...
  CASE template_record.name
    ...
    ELSE
      -- Para templates genéricos (como backup_completed_default), não fazer processamento específico
      -- As variáveis já foram processadas no loop acima
      NULL;
  END CASE;
