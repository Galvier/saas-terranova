...
-- Corrigir erro de divisão por zero na função process_automatic_notifications
...
BEGIN
  ...
  -- 2. Verificar metas atingidas recentemente (com proteção contra divisão por zero)
  FOR metric_record IN 
    ...
    WHERE md.is_active = true 
      ...
      AND md.target > 0  -- Proteção contra divisão por zero
      AND mv.value > 0   -- Proteção contra divisão por zero
      AND (
  ...
  LOOP
    -- Calcular percentual de achievement com proteção contra divisão por zero
    DECLARE
      achievement_percentage NUMERIC := 100;
    BEGIN
      IF metric_record.lower_is_better AND metric_record.current_value > 0 THEN
        achievement_percentage := ROUND(((metric_record.target / metric_record.current_value) * 100), 1);
      ELSIF NOT metric_record.lower_is_better AND metric_record.target > 0 THEN
